...
import type { NarrationChunk, Scene, SceneVisualType, MysteryProject } from "./types";
...
function buildHookClipPlans(project: MysteryProject): ClipPlan[] {
  ...
  const hookTotal = hookNarration.reduce((s: number, c: any) => s + (c.durationSeconds || 0), 0);
  ...
  const montageIds = (project.hookMontageSceneIds || []).filter(
    (id: string) => sceneById.get(id)?.visualStatus === "done" && sceneById.get(id)?.visualUrl
  );
...
const DATA_VISUAL_TYPES: SceneVisualType[] = ["data_card", "timeline", "diagram"];
...
function assertReadyForRender(project: MysteryProject): void {
